...
public class Rotor
{
	...
	public Rotor(String s)
	{
		...
		for (int i = 0; i < s.length(); i++)
		{
			...
			rightToLeftWiring[Symbol.getOffset(currentChar)] = (char) (Symbol
					.getSymbolAtOffset(i));
		}
	}
	...
	public char encodeLR(char c)
	{
		...
		// If the position causes the index to exceed the maximum number of
	...
	public char encodeRL(char c)
	{
		...
		int offset = Symbol
				.getOffset((char) (rightToLeftWiring[Symbol.getOffset(c)] - position));

		// Because position is subtracted, we instead test for less than zero
